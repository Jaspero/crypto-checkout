import {Coin} from '..';
import {PaymentMethod} from '../types/payment-method.class';

class MetaMask extends PaymentMethod {
	get eth() {
		return window.ethereum;
	}

	ethToWei(value: number) {
		return '0x' + Math.round(value * Math.pow(10, 18)).toString(16)
	}

	available(): boolean {
		return typeof this.eth !== 'undefined';
	}

	async transfer(coin: Coin, value: number) {
		const wei = this.ethToWei(value);
